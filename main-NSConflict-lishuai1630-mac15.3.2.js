      ...
      handle.onmousedown = (e) => {
        ...
        currentOnMove = (ev) => {
          ...
          if (nItems === 1) {
            ...
            const dy2 = ev.clientY - e.clientY;
            const xSign = hd.relX < 0.5 ? -1 : 1;
            const ySign2 = hd.relY < 0.5 ? -1 : 1;
            const wx2 = 2 * Math.abs(hd.relX - 0.5);
            const wy2 = 2 * Math.abs(hd.relY - 0.5);
            const SENS = 1;
            const delta = wx2 + wy2 > 0 ? (dx * xSign * wx2 + dy2 * ySign2 * wy2) / (wx2 + wy2) * SENS : 0;
            const newHeight = Math.max(50, Math.min(maxHeight, Math.round(startHeight + delta)));
            const h2 = `${newHeight}px`;
            this.container.style.height = h2;
            this.itemEls[0].style.height = h2;
            this.imageEls[0].style.height = h2;
          ...
          const sY = 1;
          const yDelta = wx + wy > 0 ? dy * ySign * wy / (wx + wy) * sY : 0;
          const newRowHeight = Math.max(50, Math.min(2e3, Math.round(startHeight + yDelta)));
